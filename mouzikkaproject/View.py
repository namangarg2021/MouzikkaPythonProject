...
class View:
    def __init__(self, top=None):
        ...
        self.songProgress.bind("<Button-1>", self.on_progress_click)
        ...
        self.nextButton.place(relx=0.796, rely=0.285, height=16, width=22)
        ...
        self._img22 = tk.PhotoImage(file="./icons/next.png")
        self.nextButton.configure(image=self._img22)
    ...
    def add_song(self):
        song_names = self.player.add_song()
        if len(song_names) == 0:
            return
    ...
    def play_song(self):
        ...
        try:
            ...
            self.setup_thread(0, self.song_length)
            self.isPlaying = True
            self.change_volume(20)
        except NoSongSelectedError as ex:
            messagebox.showerror("Error", ex)

    def setup_thread(self, curr_s, total_sec):
        self.my_thread = threading.Thread(target=self.show_timer, args=(curr_s, total_sec))
        self.my_thread.start()

    def show_timer(self, curr_s, total_sec):
        curr_sec = curr_s

        while curr_sec <= total_sec and (mixer.music.get_busy() or self.isPaused):
            if self.isPaused:
                time.sleep(1)
                continue
        ...
        # print(curr_sec, " ", total_sec)
        # print(mixer.music.get_busy(), " ", self.isPaused)
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            if (len(fav_song_index_tuple) == 0):
                raise NoSongSelectedError("Please select a song for removing from favorites")
            song_name = self.playList.get(fav_song_index_tuple[0])
    ...
    def load_songs_from_favourites(self):
        try:
            ...
            if result_str.find("No songs") != -1:
                messagebox.showinfo("Favorites Empty!!", result_str)
                return
    ...
    def on_progress_click(self, e):
        percentage_clicked = e.x / self.songProgress.winfo_width()
        new_value_seconds = round(percentage_clicked * self.song_length)

        minutes, seconds = divmod(round(new_value_seconds), 60)
        new_time = text = str(minutes) + ":" + str(seconds)
        # print(minutes, seconds)
        self.songTimePassed.config(text=new_time)
        self.isPaused = False
        if (self.isThreadRunning == False):
            self.isThreadRunning = True
        else:
            self.player.stop_song()
            time.sleep(1)
        self.player.play_from_passed_time(new_value_seconds)
        self.isPlaying = True
        self.songProgress.config(value=new_value_seconds)
        self.change_volume(20)
        self.setup_thread(new_value_seconds, self.song_length)
